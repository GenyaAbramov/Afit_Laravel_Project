...
class HomeController extends Controller
{
    ...
    public function projects($date)
    {
      ...
      return Project::whereBetween('regdate', [$first_day, $last_day])
    ...
    private function getMembersWithSlideTicketCount($date, $id)
    {
      ...
      $first_day = date('Y-m-01 00:00:00', $timestamp);
      $current_day  = date('Y-m-d 11:59:59');
      $sql = "select teammember.email, teammember.project_rowid, teammember.teammember_rowid, (SELECT COUNT(*) FROM ticket WHERE teammember_rowid = teammember.teammember_rowid AND num=1) AS ticketSlideCount, (SELECT COUNT(*) FROM ticket WHERE teammember_rowid = teammember.teammember_rowid) AS ticketCount FROM `teammember` LEFT JOIN ticket ON ticket.teammember_rowid = teammember.teammember_rowid WHERE teammember.project_rowid='".$id."' AND teammember.regdate BETWEEN '".$first_day."' AND '".$current_day."' order by ticketSlideCount DESC";
